...
- P0（契约迁移已收口）：Client 已直接消费 `ChatApp.Protocol.Tcp 0.4.2` 的历史、同步、附件与会话水位类型，本地同义 DTO 已删除；Client request/Gateway response golden 已由 Protocol tests 固定。已从 `packages/` feed locked restore `0.4.2` 并 Release 构建 `0 warning / 0 error`；Protocol `58/58`、Unit `40/40`、Integration `185/185` 全过（含关系 fail-closed 与 `SyncBootstrapMultiPageTests` 适配精简 DTO）。
